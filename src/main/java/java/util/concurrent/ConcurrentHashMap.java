...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K,V>[] tab = table;;) {
            ...
            if (tab == null || (n = tab.length) == 0)
                // 初始化容量
                tab = initTable();
            else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) { // CAS 获取当前对应的桶是否为空
                // CAS 在当前空桶中添加键值对，添加成功则退出循环
                ...
                // 锁住当前桶里的第一个节点
                synchronized (f) {
                    if (tabAt(tab, i) == f) { // f是否为第一个节点，若不是则进入下一次循环
                        if (fh >= 0) {
                            binCount = 1;
                            // 遍历链表
    ...
    private final void addCount(long x, int check) {
        ...
        CounterCell[] as;
        long b, s;
        // 计数数组为空，则cas将basecount增加x，(x为传入需要增加的个数)
        //          若cas失败，则进入条件体
        if ((as = counterCells) != null ||
            ...
            // as为空 则fullAndCount
            // 若as不为空，随机计算出as的某个下标
            //          若下标位置为空，fullAndCount
            //          否则cas将x增加的目标CounterCell中，失败则fullAndCount
